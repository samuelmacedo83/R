...
cidades_filtro <- c("S„o Caetano do Sul",
                    "Santo AndrÈ",
                    "Mau·",
                    "Diadema",
...
# Criando o data frame com a frequ√™ncia acumulada de novos casos desde o dia do primeiro caso
...
ui <- fluidPage(
  titlePanel(tags$strong("EvoluÁ„o do COVID nas 7 Cidades do ABC")), #end of titlePane
...
server <- function(input, output) {
  ...
  output$covid_linear <- renderPlot({
    ...
    # Criando o gr√°fico dos casos em fun√ß√£o das datas
    plot_shiny <- df_covid_shiny %>%
      ...
      annotate(geom = "text",
               ...
               colour = "#2769db",
               hjust = -0.01,
               label = "In√≠cio da quarentena no Estado de S√£o Paulo (25/03/2020)") +
      labs(
        x = "Data de novos casos",
        y = "N√∫mero de casos acumulados por dia (linear)",
        title = "Evolu√ß√£o do n√∫mero de casos COVID acumulados por dia nas 7 cidades do ABC",
        caption = "Dados p√∫blicos e abertos providos pelo Brasil.IO") +
      theme_bw() +
      theme(
